...
param(
    ...
    [string]$Repository = 'example/templatecli',
...
function Get-NativeCommandOutput
{
    ...
    try
    {
        ...
        if ($process.ExitCode -ne 0)
        {
            ...
            throw "Failed to run '$FilePath $($ArgumentList -join ' ')': $message"
        }
...
function Try-EnablePowerShellCompletion
{
    ...
    if (-not (Test-NativeArgumentCompleterSupported))
    {
        return [pscustomobject]@{
            ...
            Message = "PowerShell tab completion requires a shell that supports Register-ArgumentCompleter -Native. Generate a script manually with:`n  templatecli completions script pwsh"
        }
    }
    ...
    try
    {
        ...
        $scriptPath = Join-Path (Split-Path -Parent $CommandPath) 'templatecli-completions.ps1'
        $scriptContent = Get-NativeCommandOutput -FilePath $CommandPath -ArgumentList @('completions', 'script', 'pwsh')
        Set-Content -Path $scriptPath -Value $scriptContent -Encoding utf8

        $profileUpdated = $false
        if (-not (Test-ProfileContainsCompletionEntry -ProfilePath $profilePath -ScriptPath $scriptPath))
        {
            ...
            Add-Content -Path $profilePath -Value '# Added by templatecli installer for shell completion'
...
function Get-GitHubHeaders
{
    $headers = @{
        ...
        'User-Agent' = 'templatecli-install-script'
    }
...
function Invoke-GitHubAssetDownload
{
    ...
    try
    {
        ...
        Invoke-WebRequest -Uri $AssetUrl -Headers (Get-GitHubHeaders) -OutFile (Join-Path $DestinationDirectory $AssetName)
    }
    catch
    {
        throw "Failed to download asset '$AssetName' from '$AssetUrl': $($_.Exception.Message)"
    }
}
...
function Get-ReleaseForQuality
{
    ...
    foreach ($release in $allReleases)
    {
        ...
        if ($release.tag_name -like 'install-scripts-v*')
        {
            continue
        }
...
function Get-TemplateCliInstallerTrustConfiguration
{
    return [pscustomobject]@{
...
function Get-TemplateCliVersionString
{
...
function Invoke-TemplateCliInstall
{
    $architecture = Get-WindowsArchitecture
    $assetName = "templatecli-win-$architecture.zip"
    ...
    try
    {
        ...
        $destinationPath = Join-Path $installDirectory 'templatecli.exe'
        $downloadedVersion = Get-TemplateCliVersionString -BinaryPath $downloadedBinaryPath
        $installedVersion = $downloadedVersion
        Write-Verbose "Downloaded templatecli.exe version: '$downloadedVersion'."

        $shouldInstall = $true
        if (Test-Path $destinationPath)
        {
            $existingVersion = Get-TemplateCliVersionString -BinaryPath $destinationPath
            $comparison = Compare-SemanticVersion -Left $downloadedVersion -Right $existingVersion
            Write-Verbose "Existing templatecli.exe version at '$destinationPath': '$existingVersion'. Comparison result: $comparison."
            if ($comparison -le 0)
            {
                $shouldInstall = $false
                $installedVersion = $existingVersion
                Write-Host "Existing templatecli.exe version '$existingVersion' is newer than or equal to downloaded version '$downloadedVersion'; skipping overwrite."
            }
        }
        ...
        switch ($completionResult.Status)
        {
            ...
            'Unavailable'
            {
                Write-Host $completionResult.Message
            }
            'Failed'
            {
                Write-Warning "templatecli was installed, but PowerShell tab completion could not be enabled automatically: $($completionResult.Message)"
            }
        }

        Write-Host "templatecli $installedVersion is ready to use from '$installDirectory'." -ForegroundColor Green
    }
    finally
    {
