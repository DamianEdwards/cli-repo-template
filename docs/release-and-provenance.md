...
`install-scripts.yml` uses the `production` environment for the single approval gate before signing keys are accessed or the public `install-scripts` branch and snapshot tag are updated. The published `install-scripts` snapshot also includes a copy of `.github\workflows\attest-install-scripts.yml` so GitHub can run that workflow on the tag-bound snapshot ref, plus snapshot-local `.gitattributes` rules so Git does not rewrite signed PowerShell script bytes after checksums are generated. `attest-install-scripts.yml` intentionally remains ungated and only publishes attestations and the immutable release for the already-approved snapshot tag.
