...
.SYNOPSIS
    ...
    from install-templatecli.ps1.

.DESCRIPTION
    This script reads the install-templatecli.ps1 file, extracts the trust
    ...
    of truth (install-templatecli.ps1) and the two scripts cannot drift apart.
...
$installerPath = Join-Path $PSScriptRoot 'install\install-templatecli.ps1'
if (-not (Test-Path $installerPath))
{
    # Try relative to script location
    $installerPath = Join-Path (Split-Path -Parent $PSScriptRoot) 'scripts\install\install-templatecli.ps1'
}

if (-not (Test-Path $installerPath))
{
    throw "Cannot find install-templatecli.ps1 at '$installerPath'"
}
...
{
    throw "Could not find #region SharedProvenanceFunctions / #endregion SharedProvenanceFunctions markers in install-templatecli.ps1"
}
...
if (-not $signerSubject)
{
    throw "Could not extract ExpectedSignerSubject from install-templatecli.ps1"
}
...
<#
.SYNOPSIS
    Verifies Authenticode provenance of a templatecli binary.

.DESCRIPTION
    AUTO-GENERATED from install-templatecli.ps1 by Generate-VerifyProvenance.ps1.
    DO NOT EDIT THIS FILE DIRECTLY — edit install-templatecli.ps1 instead.

    This script is embedded as a resource in the templatecli binary and extracted
...
# Trust configuration — extracted from install-templatecli.ps1 (single source of truth)
...
$output = "$header`n$trustBlock`n`n# --- Trust/cert functions (extracted from install-templatecli.ps1) ---`n`n$sharedFunctions`n$footer`n"
