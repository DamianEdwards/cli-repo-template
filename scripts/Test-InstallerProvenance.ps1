...
param(
    ...
    [string]$InstallerScriptPath = (Join-Path $PSScriptRoot 'install\install-templatecli.ps1'),
...
$config = Get-TemplateCliInstallerTrustConfiguration
$tempRoot = Join-Path ([System.IO.Path]::GetTempPath()) ("templatecli-installer-verification-" + [guid]::NewGuid().ToString('N'))
